...
//
// class view_contract extends StatefulWidget {
//   String renter_id;
//   DateTime start_date;
//
//   view_contract({Key? key, required this.renter_id, required this.start_date})
//       : super(key: key);
//
//   @override
//   State<view_contract> createState() => _view_contractState();
// }
//
// class _view_contractState extends State<view_contract> {
//   int _page = 0;
...
//
//
// // Future<Object?> fetchHouseDetails(String house_id) async {
//   //   try {
//   //     final DocumentSnapshot<Map<String, dynamic>> houseSnapshot = await FirebaseFirestore.instance
//   //         .collection('renters_db') // Replace 'renters' with the actual collection name
//   //         .doc(house_id) // Replace `widget.renter_id` with the desired document ID
//   //         .get();
//   //
//   //     // Check if the renter exists and has an active status
//   //     if (houseSnapshot.exists && houseSnapshot['status'] == 'active') {
//   //       // Extract renter and house data
//   //
//   //       Map<String, dynamic>  houseNumber = houseSnapshot['house_no'];
//   //       Map<String, dynamic>  houseRent = houseSnapshot['rent'];
//   //
//   //
//   //     }
//   //
...
//   //   try {
//   //     final DocumentSnapshot<Map<String, dynamic>> houseSnapshot = await FirebaseFirestore.instance
//   //         .collection('houses_db') // Replace 'renters' with the actual collection name
//   //         .doc(house_id) // Replace `widget.renter_id` with the desired document ID
//   //         .get();
//   //
//   //     // Check if the renter exists and has an active status
//   //     if (houseSnapshot.exists && houseSnapshot.data()?['status'] == 'active') {
...
//                         final renterUrl = contractDocument['renter_signature'];
//                         final houseOwnerUrl = contractDocument['house_owner_signature'];
//
//
//                         return Column(
...
//                                           // padding: EdgeInsets.only(left: 10,right: 10),
//                                           // width: double.infinity,
//                                           child: Image.network(houseOwnerUrl)
//                                       )
//                                     ],
//                                   ),
//                                 ],
...
//   //   DocumentSnapshot snapshot =  FirebaseFirestore.collection("contracts_db")
//   //       .where('renter_id', isEqualTo: widget.renter_id)
...
class view_contract extends StatefulWidget {
  String renter_id;
  DateTime start_date;
   view_contract({Key? key, required  this.renter_id, required  this.start_date}) : super(key: key);
...
class _view_contractState extends State<view_contract> {
  ...
  //   DocumentSnapshot rachelSnapshot = await FirebaseFirestore.instance
  //       .collection('contracts_db')
  //       .doc('NKFxr7RRJtvCOlhNuXcf')
  //       .get();
  //
  //   if (rachelSnapshot.exists && rachelSnapshot['status'] == 'active') {
  //     setState(() {
  //       status = rachelSnapshot['status'];
  //     });
  //   } else {
  //     setState(() {
  //       status = 'No active data found for Rachel';
  //     });
  //   }
  // }
  ...
  void fetchHouseDetails(String house_id){
    Future<DocumentSnapshot> houseDetails = FirebaseFirestore.instance
        .collection('houses_db')
        .doc(house_id)
        .get();
    houseDetails.then((value) => {
      setState(() {
        houseNumber = value['house_no'];
          Rent = value['rent'];
      }),
  ...
  // void fetchHouseDetails(String house_id) async {
  //   DocumentSnapshot houseSnapshot =
  //   await FirebaseFirestore.instance.collection('houses_db').doc(house_id).get();
  //
  //   if (houseSnapshot.exists) {
  //     // House document exists
  //     Map<String, dynamic> houseData = houseSnapshot.data! as Map<String, dynamic>;
  //     String status = houseData['status'];
  //
  //     if (status == 'active') {
  //       // House is active
  //       // Access and use the house data as needed
  //       houseNumber = houseData['house_no'];
  //       houseRent = houseData['rent'];
  //
  //       // Perform any desired operations with the house details
  //
  //     } else {
  //       // House is not active
  //       print('House is not active');
  //     }
  //   } else {
  //     // House document does not exist
  //     print('House document does not exist');
  //   }
  // }


  // void fetchContract() async {
  //   QuerySnapshot contractSnapshot = await FirebaseFirestore.instance
  //       .collection('contracts_db')
  //       .where('renter_id', isEqualTo: widget.renter_id)
  //       .where('start_date', isEqualTo: widget.start_date)
  //       .get();
  //
  //   if (contractSnapshot.size > 0) {
  //     DocumentSnapshot contractDoc = contractSnapshot.docs[0];
  //     if (contractDoc['status'] == 'active') {
  //       setState(() {
  //
  //         paidRent = contractDoc['paid_amount'];
  //         Timestamp startTimestamp =
  //         contractDoc['start_date'];
  //         Timestamp endTimestamp = contractDoc['end_date'];
  //
  //         DateTime startDate = startTimestamp.toDate();
  //         DateTime endDate = endTimestamp.toDate();
  //       });
                         ...
                         Container(
                           child: Center(
                             ...
                             Container(
                                          ...
                                          // padding: EdgeInsets.only(left: 10,right: 10),
                                          // width: double.infinity,
                                          child: Image.network(contractDocument['renter_signature'])
                                      )
                              ...
                              Row(
                                children: [
                                  ...
                                  Container(
                                      ...
                                      // padding: EdgeInsets.only(left: 10,right: 10),
                                      // width: double.infinity,
                                      child: Image.network(contractDocument['house_owner_Signature'])
                                  )
                                ],
                              ),
                            ],
